...
function concatenateStrings(value1, value2) {
  const lengthStrValue1 = value1.length;
  const lengthStrValue2 = value2.length;
  let res = '';
  if (lengthStrValue1 === 0) {
    res = value2;
  } else if (lengthStrValue2 === 0) {
    res = value1;
  } else {
    res = value1.concat(value2);
  }
  return res;
}
...
function sumOfCodes(str) {
  if (str) {
    const arr = str.split('').map((item, index) => str.charCodeAt(index));
    const res = arr.reduce((acc, count) => {
      return acc + count;
    }, 0);
    return res;
  }
...
function formatTime(minutes, seconds) {
  const res = `${minutes.toString().padStart(2, '0')}`;
  return res.concat(`:${seconds.toString().padStart(2, '0')}`);
}
...
function extractNameFromTemplate(value) {
  ...
  const lastElemIndex = value.length - 1;
  return value.slice(whiteSpaceIndex, lastElemIndex).trim();
}
